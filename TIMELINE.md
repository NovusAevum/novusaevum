<div align="center">
  <img src="./assets/headers/timeline-header.svg" alt="Timeline Header - The Journey" width="100%"/>
</div>

# 🗺️ TIMELINE: The Journey

---

## 🌊 The Complete Journey

```mermaid
timeline
    title Transformation Timeline: 2008 - Present
    
    section Foundation (2008-2014)
        School Dropout : Crisis Point
        SPM Mastery (1 Year) : 5A, 1B, 4C
        STPM (3 Months) : CGPA 3.84
        USM Dean's List : CGPA 3.26
        
    section Construction Leadership (2015-2020)
        Lee Swee Construction : Site Supervisor
        Senior Supervisor : Leading Teams
        Site Agent/Manager : 4 Major Projects
        Leadership Forged : Execution Under Pressure
        
    section Entrepreneurial Testing (2020-2023)
        WMH Resources Founded : E-commerce Launch
        3 Online Shops : Scaling Operations
        Strong Performance : 2022 Success
        The 2023 Crisis : DOS Attack + Breakdown
        
    section Technology Transformation (2023-Present)
        Strategic Pivot : Calculated Recalibration
        100+ Certifications : Rapid Mastery
        Google Partnership : Individual Achievement
        IBM & AWS Partnerships : Multi-Cloud Excellence
        NovusAevum : A New Era Begins
```

---

## 📈 Detailed Phase Breakdown

### Phase 1: Foundation Through Adversity (2008-2014)

```mermaid
flowchart LR
    A[School Dropout<br/>2008] --> B[SPM Mastery<br/>1 Year]
    B --> C[STPM Excellence<br/>3 Months]
    C --> D[USM Dean's List<br/>CGPA 3.26]
    
    style A fill:#ff6b6b
    style B fill:#4ecdc4
    style C fill:#45b7d1
    style D fill:#96ceb4
```

**Key Learnings:**
- Failure is data, not destiny
- Learning velocity > credentials
- Resilience is a skill, not a trait
- Every comeback refines the approach

---

### Phase 2: Construction Leadership (2015-2020)

```mermaid
graph TD
    A[Lee Swee Construction<br/>Site Supervisor] --> B[Senior Supervisor<br/>Team Leadership]
    B --> C[Site Agent/Manager<br/>4 Major Projects]
    C --> D[Metro Kajang Holdings]
    C --> E[Ecoworld Bhd]
    C --> F[Multi-Million Dollar Projects]
    
    style A fill:#667eea
    style B fill:#764ba2
    style C fill:#f093fb
    style D fill:#4facfe
    style E fill:#00f2fe
    style F fill:#43e97b
```

**Skills Developed:**
- Leadership under extreme pressure
- Project management & stakeholder coordination
- Budget management & timeline execution
- Decision-making with incomplete information

---

### Phase 3: Entrepreneurial Testing Ground (2020-2023)

```mermaid
journey
    title E-commerce Journey: WMH Resources
    section Launch (2020)
      Founded WMH Resources: 5
      First Online Shop: 4
      Operations Setup: 4
    section Growth (2021-2022)
      Second Shop Launch: 5
      Third Shop Launch: 5
      Performance Peak: 5
      Sustained Success: 5
    section Crisis (2023)
      Cyber DOS Attack: 1
      Relationship Breakdown: 1
      Financial Reset: 1
      Strategic Decision: 5
```

**The 2023 Pivot:**
- Cyber warfare experience → Cybersecurity depth
- Business collapse → Strategic recalibration
- Personal crisis → Philosophical clarity
- **Not a retreat. A calculated pivot.**

---

### Phase 4: Technology Transformation (2023-Present)

```mermaid
mindmap
  root((NovusAevum<br/>2023-Present))
    AI/ML
      Google Gen AI Leader
      IBM AI/ML
      AWS Machine Learning
      RAG Pipelines
    Cybersecurity
      CEH v12
      TryHackMe CTF
      OSINT Expert
      Red Team Thinking
    Cloud Computing
      Google Cloud Professional
      AWS Partnership
      Azure Certifications
      Multi-Cloud Architecture
    Digital Marketing
      Google DM Professional
      HubSpot CRM
      Performance Marketing
      Growth Engineering
    Strategic Partnerships
      Google Partner
      IBM Business Partner
      AWS Partnership
      Individual Achievement
```

**The Acceleration:**
- 100+ certifications in compressed timeframe
- Strategic partnerships without organizational backing
- Transdisciplinary mastery across 5 domains
- **Building capital with no currency**

---

## 🎯 Key Milestones

| Year | Milestone | Impact |
|------|-----------|--------|
| 2008 | School Dropout | Learning to rebuild from nothing |
| 2009 | SPM Mastery (1 year) | 5A, 1B, 4C — Proof of learning velocity |
| 2010 | STPM Excellence (3 months) | CGPA 3.84 — Speed + depth |
| 2011-2014 | USM Dean's List | CGPA 3.26 — Academic validation |
| 2015-2020 | Construction Leadership | 4 major projects, millions in value |
| 2020 | WMH Resources Founded | First entrepreneurial venture |
| 2021-2022 | 3 Shops Scaling | E-commerce success |
| 2023 | The Crisis & Pivot | DOS attack, breakdown, reset, transformation |
| 2023-Present | 100+ Certifications | Rapid mastery across 5 domains |
| 2024 | Strategic Partnerships | Google, IBM, AWS (individual) |
| 2025 | NovusAevum Era | Transdisciplinary strategist fully realized |

---

## 📊 Growth Metrics

```mermaid
graph TD
    subgraph "2008-2014: Foundation Era"
        A[School Dropout<br/>Level: 10] --> B[SPM Mastery<br/>Level: 30]
        B --> C[STPM Excellence<br/>Level: 45]
        C --> D[USM Dean's List<br/>Level: 60]
    end
    
    subgraph "2015-2020: Execution Era"
        D --> E[Construction Leadership<br/>Level: 75]
        E --> F[WMH Resources<br/>Level: 80]
    end
    
    subgraph "2020-2023: Crisis & Pivot"
        F --> G[E-commerce Scale<br/>Level: 60]
        G --> H[DOS Attack & Reset<br/>Level: 85]
    end
    
    subgraph "2023-2025: Mastery Era"
        H --> I[100+ Certifications<br/>Level: 95]
        I --> J[Strategic Partnerships<br/>Level: 100]
    end
    
    style A fill:#ff6b6b
    style B fill:#ffa07a
    style C fill:#ffd700
    style D fill:#98d8c8
    style E fill:#4ecdc4
    style F fill:#6c5ce7
    style G fill:#a29bfe
    style H fill:#fd79a8
    style I fill:#fdcb6e
    style J fill:#00b894
```

### 📈 **Capability Evolution Over Time**

| Year | Overall Mastery | Technical Skills | Strategic Thinking | Business Acumen |
|------|----------------|------------------|-------------------|------------------|
| 2008 | 10% | 5% | 0% | 5% |
| 2010 | 30% | 15% | 5% | 10% |
| 2012 | 45% | 25% | 10% | 10% |
| 2014 | 60% | 35% | 15% | 10% |
| 2016 | 70% | 40% | 20% | 10% |
| 2018 | 75% | 45% | 20% | 10% |
| 2020 | 80% | 50% | 20% | 10% |
| 2022 | 60% | 35% | 15% | 10% |
| 2024 | 85% | 75% | 85% | 80% |
| 2025 | 95% | 90% | 95% | 90% |

**Key Insight:** Notice the dip in 2022 (crisis year), followed by **explosive growth** in 2023-2025 through focused learning and strategic partnerships.

---

## 🌟 What This Journey Taught Me

### 1. Failure is Fuel
Every collapse was a catalyst. Every breakdown was a breakthrough waiting to happen.

### 2. Speed Matters
- SPM in 1 year vs. 2 years: **2x velocity**
- STPM in 3 months vs. 18 months: **6x velocity**
- 100+ certifications in 2 years: **Unprecedented acceleration**

### 3. Resilience is Trainable
It's not about never falling. It's about the **recalibration speed** after impact.

### 4. Play Your Own Game
- Others played poker with luck
- I learned to play chess with strategy
- **Different game, different rules, better outcomes**

---

## 🎭 The Philosophy Evolution

```mermaid
graph LR
    A[Survival Mode<br/>2008-2014] --> B[Execution Mode<br/>2015-2020]
    B --> C[Entrepreneurial Mode<br/>2020-2023]
    C --> D[Strategic Mode<br/>2023-Present]
    
    style A fill:#ff6b6b
    style B fill:#4ecdc4
    style C fill:#f093fb
    style D fill:#667eea
```

**Current Philosophy:**
- "I build capital with no currency"
- "I play chess in a world built for poker"
- "Precision is my weapon. Clarity is my mandate"

---

## 🚀 What's Next?

The journey continues. The transformation never stops.

Currently exploring intersections where:
- **Innovation** meets meaningful impact
- **Technology** meets human needs
- **Strategy** meets execution
- **Vision** meets reality

---

<div align="center">

### "Every scar made my thinking clearer and my actions quieter."

---

[← Back to Main](./README.md) | [The Arsenal →](./TECHNICAL-DEPTH.md)

</div>
